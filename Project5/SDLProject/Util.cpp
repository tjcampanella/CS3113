...
void Util::DrawText(ShaderProgram *program, GLuint fontTexture, std::string text, float size,
    ...
    for(int i = 0; i < text.size(); i++) {
        ...
        modelMatrix = glm::translate(modelMatrix, position); program->SetModelMatrix(modelMatrix);
        ...
        glBindTexture(GL_TEXTURE_2D, fontTexture); glDrawArrays(GL_TRIANGLES, 0, (int)(text.size() * 6));
